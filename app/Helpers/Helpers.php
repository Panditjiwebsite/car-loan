...
function get_clubname($id) {    
        return \DB::table('club')->where('id', $id)->first()->name_en;
...
function getNotification() {
  if (!empty(session('user_data')['sub_contractors_id']) ) {
   ...
   //pr($userId);die;
   $userType = session('user_data')['user_type'];
   //pr($userType);die;
   $data = DB::table('notifications')->select('title')->where(['user_id' => $userId, 'user_type' => $userType, 'read' => 0])->count();
  }
   //pr($data);die;
...
function notificationA($registrationId, $body, $title,$usermsg,$type='', $icon='') {
  ...
  define( 'API_ACCESS_KEY', 'AAAAQX0bGC4:APA91bHwAn0mlfXzwQeK_W31GMCBpobakNrUaRWcAL6mklrEWWqyRgVRKcyU9r_5FkOLrFlKfn_EuTwpxqwFPDxAKPWx1u38ypDOBY5R5YZmivOodJv9PJn3i1T1AyG6WSmHtDvin4hT');
  ...
  #prep the bundle
    $msg = array(
      ...
      'icon'    => $icon,
      'message' => $usermsg,
      'type'    => $type //activity, session, order
    );
...
function notificationI($registrationId, $body, $title, $usermsg, $type='', $icon='') {
  ...
  //// chat notification
  function cnotificationA($registrationId, $body, $title,$userid,$firstname,$lastname,$usermsg, $type='', $icon='')  {
  ...
  function cnotificationI($registrationId, $body, $title,$userid,$firstname,$lastname,$usermsg, $type='', $icon='') {
...
function get_username($id, $user_type) {
    if($user_type == 1){
        return \DB::table('user_signup')->where('id', $id)->first()->fname;
    }
    if($user_type == 2){
